...
function grupos = runKMeans(z, num_gru)
    ...
    % === Intentar cargar centroides predefinidos ===
    if isfile('centroides_KM.txt')
        V_cargado = load('centroides_KM.txt');
        if size(V_cargado, 1) == n && size(V_cargado, 2) == num_gru
            v = V_cargado;
            disp('📂 Centroides cargados desde "centroides_KM.txt".');
        else
            warning('⚠️ Dimensiones de centroides no coinciden. Se inicializarán aleatoriamente.');
            rng('shuffle');
            pos = randperm(N, num_gru);
            v = z(:, pos);
        end
    else
        disp('⚠️ Archivo "centroides_KM.txt" no encontrado. Inicializando aleatoriamente.');
        rng('shuffle');
        pos = randperm(N, num_gru);
        v = z(:, pos);
    end

    % === Clasificación (una sola iteración, sin recalcular centroides) ===
